...
| `plot_pad` | 1.0 x 1.0 | 0.15 | Shikaku's floor slab, one per cell: `Pad_Body` (`Stone`, tinted by whether a rectangle has claimed the cell) |
...
| `clue_stone` | 0.7 x 0.7 | 0.24 | the clue's marker stone, a low hexagonal drum: `Clue_Body` (`Stone`) and `Clue_Num_1` … `Clue_Num_9` (`Num_flat`, carved numerals on the crown, as the `plinth` carries them); the board shows the one matching the rectangle's area. Six sides on purpose -- a piece is told apart by its silhouette first, and the square and the circle are taken |
