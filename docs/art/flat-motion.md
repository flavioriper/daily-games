...
the fourteenth, also built on the vocabulary rather than ported. Rings
followed on 2026-09-20 too, the fifteenth, built on the vocabulary from its
first line. (Word Trail has no row of its own in the table below yet; its
motion is recorded in CLAUDE.md and in its own spec's section 11.) This page
is the table a new or a ported board is built against. When a number here
and a number in a board disagree, the board is wrong.
