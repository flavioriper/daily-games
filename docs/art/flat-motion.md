...
the fifteenth, Quilt the sixteenth and Rings the seventeenth, all the same
day again, and all three built on the vocabulary from their first line.
(**Word Trail and Bridges have no row of their own in the table below**;
their motion is recorded in CLAUDE.md and in each spec's own section. Quilt
and Rings have one each, because each signature needed a shape the table did
not already hold.) This page is the table a new or
...
| Rings | on it (2026-09-20), built on the vocabulary from the first line: the board pops in wide about its centre and each station drops in from `DROP` above at `ENTER_STAGGER` a peg apart, exactly the family's entrance and nothing of its own. A held ring rises `LIFT_H` over its post on the back ease over `LIFT_TIME` and then breathes `BOB` on a `BOB_CYCLE` for as long as it waits, carrying the soft cast shadow every lifted or flying ring wears; under reduce motion it is simply up and does not breathe. **The flight is its signature**: a dropped ring travels rather than snapping, `x` on a sine ease between the two stations and `y` held near the lift height early and falling late (an eased arc, `ARC_LIFT` over `ARC_TIME`), clamped so it can never rise clear of the card -- found by shooting the concept page, not by reading it -- and its destination's slot is skipped in the mesh while it is airborne, or the ring would show twice; the landing squashes on the shape `Motion.squash`'s own tween plays, read as a curve the way Shikaku's beds are (rule 8), because this board draws its rings and Motion has no reader for that one recipe. A peg that has just come right washes its rings gold top-down, one `WAVE_STEP` apart, each on `flash_level` toward `Pal.SUN_RAY`, derived from `_lock_at` and nothing else, the way Queens' crosses and Sudoku's finished units are; a refused drop shivers the station on the family's own default `SHIVER_PX`, no parameter of this board's; Reset drops the pegs back in a wave at `RESET_STAGGER`, and the solve hops every peg in a wave at `SOLVE_STAGGER` after `SOLVE_DELAY`, `SOLVE_HOP` over `SOLVE_TIME`, the win following at `WIN_WAIT` (`REDUCED_TIME` under reduce motion). Undo and a hint both fly their ring exactly like a tapped drop -- Undo backwards, settling nothing, since a peg can never be newly locked by taking a ring off another; a hint plays the same two touches the solver would. `core/motion.gd` gains nothing: the wash reads `WAVE_STEP`, the entrance and solve read their own already-shared constants, and the shiver takes the default rather than a number of its own. **It is the only board in the game that can reach a position with no legal move**, and the check behind it (`is_stuck()`) is two loops and no solver; a toast -- `Pal.TEXT` under `Pal.PAPER` text, cached by its own line the way Hidden Word caches its pill -- says so over the board's foot for `TOAST_HOLD`, fading in and out over `Motion.DROP_FADE` at each edge, and is deliberately the only thing that happens: no dimming, no forced ending. Measured while the concept page was built (2026-09-20): in 450 careless games it never fired, and the true rate behind it (a position still legal and already lost) is 4%, 9% and 11% by band. Measured at 810x1440 (see CLAUDE.md, "What the harnesses actually measure"), re-read after the merge with main: **61** draw calls with a ring flying and a peg mid-wash, three runs in a row and 61 every time; **57** bare (58 once, when an fx ring was still alive on the sampled frame), 57 under reduce motion, and **35** on the win screen -- all well inside the 855 budget. **The 110 first recorded here for the win screen was wrong** and is corrected: the win screen takes the top bar and the tip card away and puts one column of five ring faces in their place, so it draws *less* than the board it covers, not twice as much. Idle 3.88, 3.66 and 3.58 ms with a move in flight against 2.31 and 2.95 ms bare, and 1.87 and 1.78 on the win screen; Word Trail, run as the control in the same hour, came back at exactly its recorded 65 draw calls twice, on idles of 3.29 and 2.54 ms against the 2.37-2.51 in its own record -- which is the usual reminder that the milliseconds off this harness are comparable only inside one session and the draw calls are the figure worth quoting. On the phone's driver (`--rendering-driver opengl3_angle`) the same 61 played and 57 bare, and a settled bare frame matches the default driver to **3/255** over 46,252 pixels, which is edge antialiasing and no `instance uniform`; a *played* frame cannot be compared across the two drivers at all, because the gold wash is mid-decay on the frame the strip samples and ANGLE runs the board at a different frame time. Two reduce-motion frames 1.5 s apart are pixel-identical.
