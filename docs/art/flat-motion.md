...
Hidden Word beside it, the eleventh. Sudoku is the twelfth, on 2026-09-20,
and also built on the vocabulary rather than ported -- **on this branch**,
before merge: parallel Word Trail work has already landed on `main` and this
count and ordinal do not see it, so whoever merges owes this line a check
and, if Word Trail wants a row here too, a rewrite of "the twelfth" and
everything counted against it. This
