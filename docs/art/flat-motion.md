...
the fifteenth, Quilt the sixteenth and Paper Planes the seventeenth, all
three the same day again and all three built on the vocabulary from their
first line. (**Word Trail and Bridges have no row of their own in the table
below**; their motion is recorded in CLAUDE.md and in each spec's own
section. Quilt and Paper Planes have one each.)
...
| Launch | **A piece leaves by travelling along its own body**, which nothing else in the game does. The piece runs a *track*: its own polyline from tail to head, extended past the head down its lane and one body-length beyond the edge of the board. With `s` the cells travelled, the body drawn is the slice of the track between `s` and `s + len - 1`, sampled at every whole number, so the tail follows the head through every bend the piece ever made rather than sliding sideways off it. It accelerates away, a puff marks where it crossed the edge, and each cell takes its dot back as the tail passes over it. An undo or a reset runs the same track the other way. **The card is the clip**: the piece is gone when it reaches the card's edge, so the board's Control is the one flat board that sets `clip_contents`. Paper Planes' (`puzzles/planes2d.gd`); nothing was added to `core/motion.gd` for it. | `LAUNCH_SPEED` 22 cells a second over `max(Motion.POP_IN, track / LAUNCH_SPEED)`; the ease is `1 - pop_out_scale` (the family's quarter-cosine read backwards) and the dot's return is `appear_level` against the second the tail crossed; `fx.puff` |
| Refused, with a path | The Refused row above, for a board where what was refused is a *route* and not a cell: the route flashes toward `BAD_TILE` from the piece as far as the thing in the way, drawn as one round-capped band a third of a cell wide -- a line, because it is the way out and not a region -- while the **blocking** piece shivers and the piece that was tapped nudges the way it wanted to go. No toast and no counter. Paper Planes'. | `flash_level` with its two halves scaled into `BLOCK_FLASH` 0.35, so the rise and fall keep the family's 0.15-against-0.45 proportion; `shiver_offset` and `nudge_offset` at the vocabulary's own `SHIVER_PX` 2 and `NUDGE` 3 |
...
| Wake | **The board answers the move**: after a piece leaves, the set of pieces that can now move is diffed against a snapshot taken before it, and every **newly freed** piece beats once where it stands, a king-move step out from the departing piece. This is Queens' `_settle` with a departure in place of a queen's sight -- and, as there, the set is **derived from the state and never stored**, so an undo leaves nothing behind to clean up and no move can beat a piece twice. Paper Planes' (its darts beat their wings: the bump whole across the spine and three tenths of it along, so a beating dart flaps rather than swells); nothing was added to `core/motion.gd` for it. | `bump_scale` `BUMP` 0.25 over `BUMP_TIME` 0.24, starting `stagger(king_distance, WAKE_STEP)` later; `WAKE_STEP` 0.04, capped at the family's 0.6 |
...
| Paper Planes | on it (2026-09-20), built on the vocabulary from the first line: the field pops in wide about its centre after `ENTER_DELAY` and each plane pops in about its **head** along a diagonal wave at `ENTER_STAGGER`, capped at the board's own `ENTER_CAP` 0.5 through `stagger`'s parameter rather than a copied constant, because fifty-two planes at the family's 0.6 is a minute of entrance; a tapped plane whose lane is clear **launches along its own body** (the Launch row above) and every plane its going freed beats its wings a king-move step later (the Wake row); a tapped plane whose lane is not launches nothing and loses nothing -- its route flashes as far as the thing in the way, that plane shivers, this one leans forward and back, and the tip card says why (the "Refused, with a path" row); a hint rings one plane that can go, beats its wings out of the same book the wake writes to, and leaves a gold wash under its whole body until it goes; Undo flies the last one home along the track it left on and Reset flies them all home in a wave from the far corner at `RESET_STAGGER`; and when the sky is empty the dots are all that is left, so the family's solve wave is a hop on each of them (`SOLVE_HOP`, `SOLVE_TIME`, `SOLVE_STAGGER`, `SOLVE_DELAY` off `hop_lift`), rolling out from the cell the last plane's head stood on and **booked for the second that last flight lands**, never for the tap -- a field hopping under a plane that has not left yet is two hands at once. Everything is drawn -- one `ArrayMesh`, no Controls, fifty-two planes -- so all of it reads the curve readers (rule 8), and **it adds nothing to `core/motion.gd`**. **Three constants of its own** and no more: `LAUNCH_SPEED` 22, `WAKE_STEP` 0.04 and `BLOCK_FLASH` 0.35; the launch's floor is `POP_IN`, its ease is `pop_out_scale` read backwards, the dot's return is `appear_level`, and the shiver and the nudge are the vocabulary's own pixels, all of which is why there is no fourth. It is also the one flat board that clips: a launch runs a body-length past the grid, and the board card is the wall it disappears behind. Draw calls and idle: its spec's section 15 |
