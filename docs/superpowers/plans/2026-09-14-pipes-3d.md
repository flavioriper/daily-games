...
static func _pipe(slot: String, mask: int) -> Node3D:
	var root := Node3D.new()
	root.name = slot
	var at_hub := Transform3D(Basis(), Vector3(0.0, TUBE_R, 0.0))
...
```python
    ...
    "pipe_cap": (1.0, 1.0, 0.35),
    "pipe_straight": (1.0, 1.0, 0.35),
    "pipe_elbow": (1.0, 1.0, 0.35),
    "pipe_tee": (1.0, 1.0, 0.35),
    "pipe_cross": (1.0, 1.0, 0.35),
...
const HEIGHT_BUDGET := {"tile": 0.9, "rim_edge": 0.12, "rim_corner": 0.12,
	...
	"pipe_pad": 0.15, "pipe_cap": 0.35, "pipe_straight": 0.35, "pipe_elbow": 0.35,
	"pipe_tee": 0.35, "pipe_cross": 0.35, "valve": 0.12}
```
...
## A piece stands 0.325 over a 0.12 pad, so a collar tops out at 0.445 and
	...
	for cell in cells:
		...
		for bit in BITS:
			...
			out.append({"cell": cell,
				"at": _cell_at(cell, Placeholders.PAD_H + Placeholders.TUBE_R)
...
func _turn(cell: Vector2i) -> void:
	...
	fx.puff(_cell_at(cell, Placeholders.PAD_H + Placeholders.TUBE_R), Pal.STEEL)
	fx.cue("turn")
...
Run: `godot --path . --resolution 1080x1920 --script res://tests/_shot.gd -- pipes`
and open `/tmp/shot_pipes.png`. Check against `docs/art/concept-pipes.png`:
...
Build the reference arm pointing at -Z, matching `core/placeholders.gd`
exactly: tube radius 0.15 with its axis at local y = 0.15; shell segments from
0.0 to 0.20 and from 0.32 to 0.498; collars of radius 0.175 and width 0.06
centred at 0.20, 0.32 and 0.468; a water tube of radius 0.115 running 0.0 to
0.498. Add a hub ball of radius 0.16 for the shell and 0.115 for the water.
...
thickness 0.003 centred on the pad diagonals at radius 0.36, laid so 0.0015
stands proud of y = 0 — the tile's inlay treatment. `Bolt_flat` ends in
`_flat`, so it carries no outline.
...
godot --path . --resolution 1080x1920 --script res://tests/_shot.gd -- pipes
```
