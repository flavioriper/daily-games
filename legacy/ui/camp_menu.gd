...
## The campsite: **the first screen until 2026-09-18**, kept whole and
## reachable from the new one's More sheet rather than deleted
## (ui/menu/legacy_sheet.gd, its last row). Its cards are the old game's:
## Registry.LEGACY, the thirteen boards on the stage and the one turn.
##
## Opened from More it is `embedded`, which gives it the back button it
## never needed as the root screen and makes it emit `closed` like any
## host. Nothing new belongs here; ui/menu.gd is the first screen now.
##
## The original doc follows.
##
## The first screen: the campsite (legacy/world/camp.gd) on the stage under the
...
const Host = preload("res://legacy/ui/island_host.gd")
const TurnHost = preload("res://legacy/ui/turn_host.gd")
const FlatHost = preload("res://ui/flat/flat_host.gd")
const CozyTheme = preload("res://ui/theme.gd")
const SafeArea = preload("res://ui/safe_area.gd")
const TitleView = preload("res://legacy/ui/hud/title_view.gd")
const IconButton = preload("res://ui/hud/icon_button.gd")
const PuzzleCard = preload("res://legacy/ui/hud/puzzle_card.gd")
const SettingsSheet = preload("res://ui/hud/settings_sheet.gd")
const Camp = preload("res://legacy/world/camp.gd")

## Emitted when the back button is pressed; only an embedded campsite has
## one. ui/menu.gd listens, frees this and puts the flat screen back.
signal closed

## Set before adding it to the tree: draws the back button and lets it be
## left. False is how it ran as the root screen.
var embedded := false
...
func _build_list() -> void:
	...
	if embedded:
		var back := IconButton.new("chevron_left")
		back.custom_minimum_size = BUTTON
	...
	for i in Registry.LEGACY.size():
		var entry: Dictionary = Registry.LEGACY[i]
