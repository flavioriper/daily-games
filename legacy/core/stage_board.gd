...
## The island boards' base: the board contract, plus the three answers a
## board on the stage gives differently. Thirteen boards and nothing else
## extend it.
##
## It carries its own copy of the contract `core/puzzle_base.gd` declares
## rather than extending it, and that is deliberate. GDScript is
## single-inheritance and these boards need the stage machinery underneath
## (`legacy/core/stage_view.gd`), which the flat boards must never load --
## that was the whole point of moving this tree. A frozen copy also means a
## later change to the live contract cannot break thirteen retired boards:
## legacy is finished, and it keeps the interface it was written against.
## Read core/puzzle_base.gd for what each method is for; the comments there
## are the ones that are maintained.
...
# --- what a board on the stage answers differently ---
